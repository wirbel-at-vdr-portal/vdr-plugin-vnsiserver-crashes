...
class cRequestPacketOld : public ICommand
{
public:
  cRequestPacketOld(uint32_t requestID, uint32_t opcode, uint8_t* data, size_t dataLength);
  ~cRequestPacketOld();
...
private:
  cRequestPacketOld( const cRequestPacketOld& other ) = delete;

  cRequestPacketOld&
  operator=( const cRequestPacketOld& other ) = delete;
